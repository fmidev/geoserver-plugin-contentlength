 ...
 * 
 * {ContentLengthResponse} class is meant to be created by its own package classes
 * and it is provided as {org.geoserver.ows.Response} class for other framework classes.
 * This {org.geoserver.ows.Response} class wrapper is used by {ContentLengthDispatcherCallback}
 * class.
 */
class ContentLengthResponse extends Response {
    ...
    /**
     * Content-Length of the output stream.
     * Negative value means that the length has not been set.
     */
     ...
     * 
     * @param {Operation} operation The operation for the request.
     *                              May be {null}.
     * @param {Object} result Operation result object whose content size is measured.
     *                        May be {null}.
     * @param {Response} response The response to the operation. 
     *                            May be {null}.
     * @throws {IOException} Any I/O errors that occur.
     */
    private void setContent(
            final Operation operation, final Object result, final Response response)
            throws IOException {
     ...
     * 
     * @param {Response} The response object that should be wrapped.
     *                   May not be {null}.
     */
     ...
     * <p>
     * This function gets {result} content size which will be set
     * for the Content-Length header when framework sets headers
     * for the response.
     * </p>
     * <p>
     * Notice, stream needs to be read into byte array if this function
     * is used. Then, excess memory needs to be used to temporarily save
     * stream content.
     * </p>
     * 
    ...
    public boolean setContentLength(
        ...
        if (null != request && null != operation && null != result
                && null != response && response.canHandle(operation)) {
            final HttpServletResponse httpResponse = request.getHttpResponse();
            if (null != httpResponse
                    && !httpResponse.containsHeader(CONTENT_LENGTH_HEADER)) {
     ...
     * <p>
     * The function uses the byte array {content} if it is available.
     * Also, if {content} is used here, reference is set to {null} and
     * the content is not available after that. The reason for this
     * is to relese the content that is originally gotten from the
     * stream to get the content length. Because content providers may
     * provide the content stream only once, the content needs to be
     * hold temporarily until it is asked once more by the framework.
     * After that, original functionality of the framework may be used.
     * Then, framework flow acts as the wrapped {org.geoserver.ows.Response}
     * would have been used without the content length operation in the
     * middle of the flow.
     * </p>
     * <p>
     ...
     * <p>
     * If content header has been set, this function will append Content-Length
     * header to the 2D header array that is provided by the {Response} class
     * instance that this class instance wraps.
     * </p>
     * 
     * @param value The value to serialize
